...
- contact, location, web_app параметры для кнопок опциональны, по умолчанию создаётся объект обычной кнопки. 
- resize_keyboard параметр клавиатуры опциональный, по умолчанию True.
и ко всем её кнопкам. Т.е. если мы хотим клавиатуру с индивидуальным расположением кнопок, но все кнопки одного типа, то мы вызываем метод get_custom_markup где явно указываем
список расположения кнопок, список названия кнопок и по желанию тип кнопок (по умолчанию обычные)/параметры клавиатуры. 
...
а остальные по одной, при этом кнопки будут тех типов, которые мы указали в словаре.
